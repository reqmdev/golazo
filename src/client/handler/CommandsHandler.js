...
const { applySlashLocalizations } = require('../../i18n/discordLocalizations');
const { isDevCommandsEnabled, isDevCommandName } = require('../../utils/devCommands');
            ...
            for (const file of readdirSync(appPath('src', 'commands', directory)).filter((f) => f.endsWith('.js'))) {
                try {
                    ...
                    if (module.__type__ === 2) {
                        ...
                        if (!isDevCommandsEnabled() && isDevCommandName(module.command.name)) {
                            info('Skipped developer application command (disabled): ' + file);
                            continue;
                        }

                        this.client.collection.application_commands.set(module.command.name, module);
                        this.client.rest_application_commands_array.push(applySlashLocalizations(module.command));
